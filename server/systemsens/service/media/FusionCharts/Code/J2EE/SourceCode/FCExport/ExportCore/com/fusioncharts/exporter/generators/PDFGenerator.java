...
public class PDFGenerator {
	...
	private byte[] addImageToPDF(int id, boolean isCompressed) {
		...
		try {
			...
			String imgObj = imgObjNo
					...
					+ metadata.getWidth() + " /Height " + metadata.getHeight()
					+ " /Length " + length + " >>\nstream\n";
	...
	public byte[] getPDFObjects(boolean isCompressed) {
		...
		try {
			...
			for (int itr = 0; itr < pageIndex; itr++) {
				...
				iWidth = metadata.getWidth();
				iHeight = metadata.getHeight();
	...
	private String getXObjResource(int itr) {
		...
		return (((itr + 2) * 3) - 1)
				+ " 0 obj\n<< /Length "
				+ (24 + ("" + metadata.getWidth() + metadata.getHeight())
						.length()) + " >>\nstream\nq\n" + metadata.getWidth()
				+ " 0 0 " + metadata.getHeight() + " 0 0 cm\n/R" + (itr + 1)
				+ " Do\nQ\nendstream\nendobj\n";
	}
