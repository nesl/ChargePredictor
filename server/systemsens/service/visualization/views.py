...
def get_database():
    conn =  _mysql.connect('localhost','root','neslrocks!','service')
    return conn

def get_model(request):
    #return HttpResponse("Hi",status=200)
    try:
        user_imei = request.POST['imei']
        user_version = request.POST['version']
        tree_diagram_mode = request.POST['tree']
    except (KeyError):
        return HttpResponseBadRequest()

    user_imei = uni2ascii(user_imei)
    user_version = int(uni2ascii(user_version))
    tree_diagram_mode = int(uni2ascii(tree_diagram_mode))

    model_db = get_database()
    model_db.query('SELECT * FROM user_models WHERE imei=' + user_imei)
    rawdata = model_db.store_result()
    table_size = rawdata.num_rows()

    database_version = -1
    updating_flag = 0

    if(table_size != 0):
        user_information = rawdata.fetch_row()[0]
        database_version = int(user_information[2])
        updating_flag = int(user_information[4])
    else:
        model_db.query('INSERT INTO user_models (imei) VALUES(' + user_imei + ');')

    if(updating_flag == 1):
        return HttpResponse("Model is updating, please wait.",status=200)
    elif(database_version > user_version):
        if(tree_diagram_mode == 1):
            return HttpResponse(user_information[3],status=201)
        return send_model(user_imei)
    else:
        generate = Thread(target=generate_model, args=(user_imei,user_version,model_db,tree_diagram_mode))
        generate.start()
        return HttpResponse("Began model update process!",status=202)
...
#    else:
#        model_db.query('INSERT INTO user_models (imei) VALUES(' + imei + ');')
#        generate = Thread(target=generate_model, args=(imei,0,model_db))
#        generate.start()
#        return HttpResponse(status=202)
...
#            if(model_update == 1):
#                return HttpResponse(status=418) # something to indic
#            else:
#                return send_model(imei)
#    return HttpResponseNotFound()
#
# Packages model file and returns as attachment content
def send_model(imei):
    mimetypes.init()
    try:
        ...
        if mimetype_guess is not None:
            response = HttpResponse(fsock, mimetype=mimetype_guess[0], status=201)
...
def generate_model(imei,version,database,td_mode):
    # Set updating flag | currently file_path is deprecated, using absolute path
    update_db(imei,'updating',1,database)
    #file_path = os.path.dirname(os.path.abspath(__file__))

    analyze.feature_generation(imei)
    update_db(imei,'err',1,database) 

    shellscript = 'run_model.sh'    
    if(td_mode == 1):
        shellscript = 'tree.sh'

    subprocess.call('/opt/systemsens/service/visualization/REU/' + shellscript + ' ' + imei,shell=True)
    
    new_version = int(version) + 1
    update_db(imei,'vers',new_version,database)
